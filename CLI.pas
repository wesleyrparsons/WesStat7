...
var
  ...
  i, Pos, SlashMark: Integer;                         // Pos traverses the Line.
  AddError, Okay, IsVar, HitOptions, InArguments, Found: Boolean;    // Flag true if in arguments before SlashMark + 1, the objects.
  EvalLine, Col: WVectorType;
  WQ1, WQ2, WQ3, WL1, WL2, WL3, WL4: FloatType;
  Lower, Upper, AInv, Q, RR, C: WMatrixType;
  U, S, VT: CMatrixType;
  WVar: IVectorType;
  OPrecision, OWidth, OReps, OSampleSize, OSampleCutoff, OPermutations: Integer;
  OSample, OExt, OUseIntercept: Boolean;
  OMissingData: MissingDataType;
  r, adjr, cov, v1, v2: FloatType;
  Cond: FloatType;
  rLower90, rUpper90, rLower95, rUpper95, rLower99, rUpper99: FloatType;                          // Error messages.
  ErrMess: Array[0..18] of String = (' is not an option', ' is not a command',                    //  0  1
    ...
    'Input is required.', 'At least two variables are required.',                                 // 14 15
    'Two integers are required.', 'One variable and a number between 0 and 1 are required1.',     // 16 17
    'One variable and a number 0 or greater are required1.');                                     // 18
...
procedure PutTokensIntoWVar(const NoneIsAll: Boolean);
var
  i: Integer;
Begin
  if NoneIsAll then
    if nArg = 0 then nArg := nCol
    else
  else
    if nArg < 1 then Exit;
  SetLength(WVar, nArg);
...
// Error message if too many args.
procedure EmptyTokenError(const x: Integer);
begin
  if (WTokenArg[x].S <> EmptyStr) and (iOpt = 0) then
    writeln(ErrMess[8]);     // Additional arguments are ignored.
end;
...
begin
  ...
  Case UpCase(WTokenArg[iOpt].S) of
    ...
    'NULL':;       // Does nothing; used in CLI.
    'NOTHING': writeln('Nothing does nothing.');
    'DISPLAYWVAR', 'WVAR' : begin DisplayWVar; EmptyTokenError(1); end;
    'SIGNIFICANCE', 'SIG': begin ShowSignificance := True; writeln('Significance: ', ShowSignificance); EmptyTokenError(1); end;
    'NOSIGNIFICANCE', 'NOSIG': begin ShowSignificance := False; writeln('Significance: ', ShowSignificance); EmptyTokenError(1); end;
    'INTERCEPT', 'INT': begin UseIntercept := True; writeln('Use intercept: ', UseIntercept); EmptyTokenError(1); end;
    'NOINTERCEPT', 'NOINT': begin UseIntercept := False; writeln('Use intercept: ', UseIntercept);  EmptyTokenError(1); end;
    'EXTENDED', 'EXT': begin Ext := True; writeln('Statistics extended.'); EmptyTokenError(1); end;
    'NOEXTENDED', 'NOEXT': begin Ext := False; writeln('Statistics not extended.'); EmptyTokenError(1); end;
    'SAMPLE', 'SAM': begin Sample := True; writeln('Sample: ', Sample); EmptyTokenError(1); end;
    'POPULATION', 'POP': begin Sample := False; writeln('Sample: ', Sample); EmptyTokenError(1); end;
    'MDMEAN': begin MissingData := MDMean; ConvertNaNinData; EmptyTokenError(1); end;  // Add writeln to show state.
    'MDMEDIAN', 'MDMED': begin MissingData := MDMedian; ConvertNaNinData; EmptyTokenError(1); end;
    'MDZERO', 'MD0': begin MissingData := MDZero; ConvertNaNinData;  EmptyTokenError(1); end;

    // For options that take one number argument. If Main Case (iOpt = 0) and WTokenArgs > 1, then show error.
    'PRECISION', 'PREC': if WTokenArg[iOpt + 1].I > 0 then begin   // Show error if more than 1 arg.
    ...
    'MAXITERATIONS', 'MAXITER', 'ITER': if WTokenArg[iOpt + 1].I > 0 then begin
        UserMaxIter := WTokenArg[iOpt + 1].I;
        writeln('Local maximum iterations = ', UserMaxIter);
        EmptyTokenError(2);
    end
    else writeln('Positive integer required (default from 50 to 5000).');
    'FASTMAXITERATION', 'FASTMAXIT': if WTokenArg[iOpt + 1].I > 0 then begin
        FastMaxIter := WTokenArg[iOpt + 1].I;
        writeln('Fast Maximum Iterations: ', FastMaxIter);
        EmptyTokenError(2);
    end
    else writeln('Positive integer required (default 50).');
    'MEDIUMMAXITERATION', 'MEDTMAXIT': if WTokenArg[iOpt + 1].I > 0 then begin
        MediumMaxIter := WTokenArg[iOpt + 1].I;
        writeln('Medium Maximum Iterations: ', MediumMaxIter);
        EmptyTokenError(2);
    end
    else writeln('Positive integer required (default 400).');
    'SLOWMAXITERATION', 'SLOWTMAXIT': if WTokenArg[iOpt + 1].I > 0 then begin
        SlowMaxIter := WTokenArg[iOpt + 1].I;
        writeln('Slow Maximum Iterations: ', SlowMaxIter);
        EmptyTokenError(2);
    end
    else writeln('Positive integer required (default 2000).');
    'VERYSLOWMAXITERATION', 'VERYSLOWTMAXIT': if WTokenArg[iOpt + 1].I > 0 then begin
        VerySlowMaxIter := WTokenArg[iOpt + 1].I;
        writeln('Very Maximum Iterations: ', VerySlowMaxIter);
        EmptyTokenError(2);
    end
    else writeln('Positive integer required (default 5000).');
    'HIGHTOLERANCE', 'HITOL': if (WTokenArg[iOpt + 1].F > 0.0) and (WTokenArg[iOpt + 1].F < 1.0) then begin
        HighTolerance := WTokenArg[iOpt + 1].F;
        writeln('High Tolerance = ', HighTolerance);
        EmptyTokenError(2);
    end
    else writeln('Number between 0 and 1 required (default from 1e-18 to 1e-6).');
    'LOWTOLERANCE', 'LOTOL': if (WTokenArg[iOpt + 1].F > 0.0) and (WTokenArg[iOpt + 1].F < 1.0) then begin
        LowTolerance := WTokenArg[iOpt + 1].F;
        writeln('Low Tolerance = ', LowTolerance);
        EmptyTokenError(2);
    end
    else writeln('Number between 0 and 1 required (default from 1e-12 to 1e-4).');
    'TOLERANCE', 'TOL': if (WTokenArg[iOpt + 1].F > 0.0) and (WTokenArg[iOpt + 1].I < 1.0) then begin
      UserTolerance := WTokenArg[iOpt + 1].F;
      writeln('Local Tolerance = ', UserTolerance);
      EmptyTokenError(2);
    end
    else writeln('Number between 0 and 1 required (default from 1e-15 to 1e-4).');
    'REPITITIONS', 'REPS': if WTokenArg[iOpt + 1].I > 0 then begin
        Reps := WTokenArg[iOpt + 1].I;
        writeln('Repititions: ', Reps);
        EmptyTokenError(2);
    end
    else writeln('Positive integer required (default 1000).');
    'SAMPLESIZE', 'SAMSIZE': if WTokenArg[iOpt + 1].I > 0 then begin
        SampleSize := WTokenArg[iOpt + 1].I;
        writeln('Sample Size: ', SampleSize);
        EmptyTokenError(2);
    end
    else writeln('Positive integer required (default 30).');
    'SAMPLECUTOFF', 'SAMCUT': if WTokenArg[iOpt + 1].I > 0 then begin
        SampleCutoff := WTokenArg[iOpt + 1].I;
        writeln('Sample Size Cutoff: ', SampleCutoff);
        EmptyTokenError(2);
    end
    else writeln('Positive integer required (default 2000).');
    'PERMUTATIONS', 'PERMS': if WTokenArg[iOpt + 1].I > 0 then begin
        Permutations := WTokenArg[iOpt + 1].I;
        writeln('Permutations: ', Permutations);
        EmptyTokenError(2);
    end
    else writeln('Positive integer required (default 500).');
    'BINCOUNT': if WTokenArg[iOpt + 1].I > 0 then begin
        BinCount:= WTokenArg[iOpt + 1].I;
        writeln('Bin Count: ', BinCount);
        EmptyTokenError(2);
    end
    else writeln('Positive integer required (default sqrt(observations)).');
    'BINNUMBER', 'BINNUM': begin
      Found := True;
      if (WTokenArg[iOpt + 1].S <> EmptyStr) and (WTokenArg[iOpt + 1].I > 0) then
        Case UpCase(WTokenArg[iOpt + 1].S) of
          'SQROOT': EntropyAlgo := SqRoot;
          'FD': EntropyAlgo := FD;
          'RICE': EntropyAlgo := Rice;
          'STURGES': EntropyAlgo := Sturges;
          'SCOTT': EntropyAlgo := Scott;
          'FIXED': EntropyAlgo := Fixed;
          else Found := False;
        end
      else
        if WTokenArg[iOpt + 1].I > 0 then begin
          BinCount := WTokenArg[iOpt + 1].I;
          Found := True;
          writeln('Bins: ', BinCount);
        end
        else writeln('Positive integer or bin number required (default sqrt(observations)).');

      if not Found then writeln('Positive integer or bin number required (default sqrt(observations)).');
    end;
    else HitOptions := False;     // No hit on cases, then HitOptions is False.
  end;
...
begin
  ...
  Case UpCase(WTokenArg[0].S) of
    ...
    'SPACES', 'SP': begin Spaces; EmptyTokenError(1); end;
    'PRINCIPALCOMPONENT', 'PRINCIPALCOMPONENTCORR', 'PCA', 'PCACORR': if not (WTokenArg[1].S = EmptyStr) and not (WTokenArg[1].S = EmptyStr) then begin
      PCAOption := CorrPCA;
      PutTokensIntoWVar(True);
      PCAnalysis(WVar);
    end
    else writeln(ErrMess[15]);
    'PRINCIPALCOMPONENTCOV', 'PCACOV': if not (WTokenArg[1].S = EmptyStr) and not (WTokenArg[1].S = EmptyStr) then begin
      PCAOption := CovPCA;
      PutTokensIntoWVar(True);
      PCAnalysis(WVar);
    end
    else writeln(ErrMess[15]);
    'DETERMINANT', 'DET': begin writeln('The determinant is ', Determinant(WData), '.'); EmptyTokenError(1); end;
    ...
    'FROBENIUS', 'FRO': begin writeln('The Frobenius Norm is ', FrobeniusNormW(WData), '.'); EmptyTokenError(1); end;
    'MATRIX', 'MAT': begin MatrixCharacteristics; EmptyTokenError(1); end;
    'TRACE', 'TR': begin writeln('The trace is ', Trace(WData), '.'); EmptyTokenError(1); end;
    'RANK': begin writeln('The rank is ', MatrixRank(WData), '.'); EmptyTokenError(1); end;
    'SYMMETRIC', 'SYM': begin
      write('The matrix is ');
    ...
    'Q', 'QUIT', 'X', 'EXIT': begin ManuallyInputData; EmptyTokenError(1); end;
    'MANUALINPUT','MANINPUT': begin ManuallyInputData; EmptyTokenError(1); end;
    'FILLCONST', 'FILLCONSTANT': begin FillScalars; EmptyTokenError(1); end;
    'PCONST', 'PCONSTANT': begin DisplayAllScalars(T2); EmptyTokenError(1); end;
    'PVAR', 'PVARIABLE' : begin for i := 0 to nCol - 1 do DisplayVectorM(WData[i].Name, WData[i], T2);EmptyTokenError(1); end;
    'ERROR', 'ERR': begin DisplayErrorStackM; EmptyTokenError(1); end;
    'I', 'INF': begin ReportProgramState; EmptyTokenError(1); end;
    'D': begin
      DebugOn := not DebugOn;
      write('Debug is '); if DebugOn then writeln('on.') else write ('off.');
      EmptyTokenError(1);
    end;
    'VERBOSE', 'V': begin Verbose := True; EmptyTokenError(1); end;
    'NOVERBOSE', 'NOV': begin Verbose := False; EmptyTokenError(1); end;
    'OPTIONS', 'OPT', 'OPTS', 'SETTING': begin ShowOptions; EmptyTokenError(1); end;
    'P1': begin DisplayMatrixM('Main Data', WData, T1); DisplayAllScalars(T1);  EmptyTokenError(1); end;
    'P', 'P2': begin DisplayMatrixM('Main Data', WData, T2); DisplayAllScalars(T2); EmptyTokenError(1); end;
    'P3': begin DisplayMatrixM('Main Data', WData, T3); DisplayAllScalars(T3);  EmptyTokenError(1); end;
    'C1': begin DisplayAllScalars(T1); EmptyTokenError(1); end;
    'C', 'C2': begin DisplayAllScalars(T2); EmptyTokenError(1); end;
    'C3': begin DisplayAllScalars(T3); EmptyTokenError(1); end;
    'MAINDATA', 'MAIN': begin DisplayMatrixM('Main Data ', WData, T2); EmptyTokenError(1); end;
    'TESTING','TEST': Begin TestRoutines; EmptyTokenError(1); end;
    'T1' : begin GetTestData(WTokenArg[0].S); EmptyTokenError(1); end;
    'T2' : begin GetTestData(WTokenArg[0].S); EmptyTokenError(1); end;
    'T3' : begin GetTestData(WTokenArg[0].S); EmptyTokenError(1); end;
    'T4' : begin GetTestData(WTokenArg[0].S); EmptyTokenError(8); end;
    'T5' : begin GetTestData(WTokenArg[0].S); EmptyTokenError(8); end;
    'T6' : begin GetTestData(WTokenArg[0].S); EmptyTokenError(8); end;
    'T7' : begin GetTestData(WTokenArg[0].S); EmptyTokenError(8); end;
    'T8' : begin GetTestData(WTokenArg[0].S); EmptyTokenError(8); end;
    'T9' : begin GetTestData(WTokenArg[0].S); EmptyTokenError(8); end;
    'T10' : begin GetTestData(WTokenArg[0].S); EmptyTokenError(8); end;
    'T11' : begin GetTestData(WTokenArg[0].S); EmptyTokenError(8); end;

    // For regression commands, which take more than one argument.
    'OLSREGRESSION', 'REGRESSION', 'OLSREG', 'OLS', 'REG': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
        RegressionMode := MLEOLS;
        PutTokensIntoWVar(False);
        MainRegression(WVar);
      end
    else writeln(WTokenArg[i].S, ErrMess[15]);
    'MLEREGRESSIONGD','GRADIENTDESCENT', 'MLEGD', 'GD': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
        RegressionMode := MLEGD;
        PutTokensIntoWVar(False);
        MainRegression(WVar);
    end
    else writeln(WTokenArg[i].S, ErrMess[15]);
    'MLEREGRESSIONNR', 'NEWTONRAPHSON', 'MLENR', 'NR': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
        RegressionMode := MLENR;
        PutTokensIntoWVar(False);
        MainRegression(WVar);
      end
      else writeln(WTokenArg[i].S, ErrMess[15]);
    'MLEREGRESSIONNEST', 'NESTEROV', 'MLENEST', 'NEST': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
        RegressionMode := MLENest;
        PutTokensIntoWVar(False);
        MainRegression(WVar);
      end else writeln(WTokenArg[i].S, ErrMess[15]);

    // For functions that take one variable (and can be multiple variables in sequence).
    'NOMINAL', 'NOM': if FoundVar(WTokenArg[i]) then NominalStatistics(WVar) else writeln(WTokenArg[i].S, ErrMess[1]);
    'UNIVARIATE', 'U', 'UNI': for i := 1 to nArg do if FoundVar(WTokenArg[i]) then DisplayUnivariateStatistics(WData[i]) else writeln(WTokenArg[i].S, ErrMess[1]);

    // The following use WVar.
    'CORRELATIONMATRIX', 'PEARSONMATRIX', 'RMAT', 'CORRELATION', 'CORR': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      Corr := Pearson;
      PutTokensIntoWVar(True);
      MultivariateCorrelations(WVar);
    end
    else writeln(ErrMess[15]);
    'SPEARMANMATRIX', 'SMAT': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      Corr := Spearman;
      PutTokensIntoWVar(True);
      MultivariateCorrelations(WVar);
    end
    else writeln(ErrMess[15]);
    'KENDALLMATRIX', 'KMAT': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      Corr := Kendall;
      PutTokensIntoWVar(True);
      MultivariateCorrelations(WVar);
    end
    else writeln(WTokenArg[i].S, ErrMess[15]);
    'HOEFFDINGMATRIX', 'HMAT': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      Corr := Hoeffding;
      PutTokensIntoWVar(True);
      MultivariateCorrelations(WVar);
    end
    else writeln(ErrMess[15]);
    'MUTUALINFORMATIONMATRIX', 'MIMAT': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      Corr := MI;
      PutTokensIntoWVar(True);
      MultivariateCorrelations(WVar);
    end
    else writeln(ErrMess[15]);
    'NORMALIZEDMUTUALINFORMATIONMATRIX', 'NMIMAT': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      Corr := NMI;
      PutTokensIntoWVar(True);
      MultivariateCorrelations(WVar);
    end
    else writeln(ErrMess[15]);
    'COVARIANCEMATRIX', 'COVMAT': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      Corr := Covar;
      PutTokensIntoWVar(True);
      MultivariateCorrelations(WVar);
    end
    ...
    // For command with two variable arguments.       Need to add error if more than two.
    'BIVARIATE', 'B', 'BIV': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      ClrScr;
      writeln('Bivariate Statistics');
      writeln('First Variable ', WData[WTokenArg[1].V].Name, ' with ', Length(WData[WTokenArg[1].V].Value), ' observations.');
      writeln('Second Variable ', WData[WTokenArg[2].V].Name, ' with ', Length(WData[WTokenArg[2].V].Value), ' observations.');
      BivariateCorrelations(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
    end
    else writeln(WTokenArg[i].S, ErrMess[10]);
    'PEARSON', 'PEARSONSR', 'CORRCOEFF': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
    ...
    'COVARIANCE', 'COV': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
    ...
    'SPEARMAN', 'SPEARMANSRHO', 'SPEAR': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
    ...
    'KENDALL', 'KENDALLSTAU', 'KENDTAU': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
    ...
    'HOEFFDINGSD', 'HOEFFDING', 'HOEFF': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
    ...
    'MUTUALINFORMATION', 'MUTUAL', 'MI': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
    ...
    'NORMALIZEDMUTUALINFORMATION', 'NORMMUTUAL', 'NMI': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
    ...
    'DISTANCECORRELATION', 'DISTCORR', 'DC': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      r := DistanceCorrelation(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
      Writeln('Distance Correlation = ', r:2 :Precision);
    end else writeln(WTokenArg[i].S, ErrMess[10]);
    'JENSENSHANNONDIVERGENCE', 'JENDIV', 'JSDIV': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      v1 := JensenShannonDivergence(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
      Writeln('Jensen Shannon Divergence = ', V1:2 :Precision);
    end else writeln(WTokenArg[i].S, ErrMess[10]);
    'JENSENSHANNONDISTANCE', 'JENDIST', 'JSDIST': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      v2 := JensenShannonDistance(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
      Writeln('Jensen Shannon Distance = ', V2:2 :Precision);
    end else writeln(WTokenArg[i].S, ErrMess[10]);
    'KULLBACKLEIBERDIVERGENCE', 'KULLDIV', 'KLDIV': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      r := KLDivergence(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
      Writeln('Kullback Leibler Divergence = ', r:2 :Precision);
    end else writeln(WTokenArg[i].S, ErrMess[10]);
    'HELLINGERDISTANCE', 'HELLDIST', 'HDIST': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      R := HellingerDistance(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
      Writeln('Hellinger Distance = ', r:2 :Precision);
    end else writeln(WTokenArg[i].S, ErrMess[10]);
    'TOTALVARIATIONDISTANCE', 'TOVARIST', 'TVDIST': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      r := TotalVariationDistance(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
      Writeln('Total Variation Distance = ', r:2 :Precision);
    end else writeln(WTokenArg[i].S, ErrMess[10]);
    'BHATTACHARYYACOEFFICIENT', 'BHATTCOEFF', 'BC': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      r := BhattacharyyaCoefficient(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
      Writeln('Bhattarchayya Coefficient = ', r:2 :Precision);
    end else writeln(WTokenArg[i].S, ErrMess[10]);
    'BHATTACHARYYADISTANCE', 'BHATTDIST', 'BD': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      r := BhattacharyyaDistance(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
      Writeln('Bhattarchayya Distance = ', r:2 :Precision);
    end else writeln(WTokenArg[i].S, ErrMess[10]);
    'CHISQUAREDIVERGENCE', 'CHIDIV', 'CHID': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      r := ChiSquareDivergence(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
      Writeln('Chi Square Divergence = ', r:2 :Precision);
    end else writeln(WTokenArg[i].S, ErrMess[10]);
    'CANBERRADISTANCE', 'CANBDIST', 'CD': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      r := CanberraDistance(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
      Writeln('Canberra Distance = ', r:2 :Precision);
    end else writeln(WTokenArg[i].S, ErrMess[10]);
    'MANNWHITNEYU', 'MWU': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      MannWhitneyU(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
    end else writeln(WTokenArg[i].S, ErrMess[10]);
    'SIGNTEST', 'ST': if FoundVar(WTokenArg[1]) and FoundVar(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then begin
      SignTest(WData[WTokenArg[1].V], WData[WTokenArg[2].V]);
    end else writeln(WTokenArg[i].S, ErrMess[10]);

    // For commands with two number arguments.
    'CREATE', 'CR' : if (WTokenArg[1].I > 0) and (WTokenArg[2].I > 0) and (WTokenArg[3].S = EmptyStr) then begin
      nCol := WTokenArg[1].I;
      nRow := WTokenArg[2].I;
      CreateRandomData;
    end
    else writeln(WTokenArg[i].S, ErrMess[16]);
    'WINSORIZE', 'WIN': if FoundVar(WTokenArg[1]) and (WTokenArg[2].F > 0.0) and (WTokenArg[3].F > 0.0) and (WTokenArg[4].S = EmptyStr) then begin  // Check args.
    ...
    'TRIM': if (WTokenArg[1].I <= nRow - 1) and FoundVar(WTokenArg[2]) and FoundVar(WTokenArg[3]) and (WTokenArg[4].S = EmptyStr) then
    ...
    'PATH': if not (WTokenArg[1].S = EmptyStr) then begin
      FilePath := WTokenArg[1].S; writeln('Path = ', FilePath); end else writeln(ErrMess[14]);
    'L': if not TeeActive then StartTeeOutput('log.txt') else StopTeeOutput;
    'LOG': if not (WTokenArg[1].S = EmptyStr) then StartTeeOutput(WTokenArg[1].S) else writeln(ErrMess[6]);
    'NOLOG': if WTokenArg[1].S = EmptyStr then StopTeeOutput else writeln(ErrMess[8]);
    ...
    'YEOJOHNSON', 'YJ', 'YEO': if FoundVar(WTokenArg[1]) and (WTokenArg[2].I > 0) and (WTokenArg[3].S = EmptyStr) then begin
        YeoJohnsonTransform(WData[WTokenArg[1].V], WTokenArg[2].F);                         // Ck args.
        writeln('Data have been been transformed by Yeo-Johnson.');
    end
    else writeln(ErrMess[5]);
{    'SHANNONENTROPY', 'SHAN', 'ENTROPY', 'ENT' : if FoundVar(WTokenArg[1]) and FoundNumber(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then
      writeln(WData[WTokenArg[1].V].Name, ' Shannon Entropy = ', SmartFloat(ShannonEntropy(WData[WTokenArg[1].V], WTokenArg[2].I)), '  ')
    else writeln(ErrMess[12]); }
    'GENERALIZEDMEAN', 'GENMEAN': if FoundVar(WTokenArg[1]) and (WTokenArg[2].F >= 0.0) and (WTokenArg[3].S = EmptyStr) then
      writeln(WData[WTokenArg[1].V].Name, ' Generalized Mean with d = ', WTokenArg[2].F: 4: 2, ': ',
        SmartFloat(GeneralizedMean(WData[WTokenArg[1].V], WTokenArg[2].F)))
    else writeln(ErrMess[18]);
    'QUANTILE', 'QUANT': if FoundVar(WTokenArg[1]) and FoundNumber(WTokenArg[2]) and (WTokenArg[3].S = EmptyStr) then
      writeln(WData[WTokenArg[1].V].Name, ' Quantile with p = ', SmartFloat(WTokenArg[2].F), ': ',
        SmartFloat(Quantile(WData[WTokenArg[1].V], WTokenArg[2].V)))
    else writeln(ErrMess[6]);
    'PERCENTILE', 'PERCENT': if FoundVar(WTokenArg[1]) and (WTokenArg[2].I > 0.0) and (WTokenArg[2].I < 1.0) and (WTokenArg[3].S = EmptyStr) then
        writeln(WData[WTokenArg[1].V].Name, ' Percentile at ', SmartFloat(WTokenArg[2].F), '%: ',
        SmartFloat(Percentile(WData[WTokenArg[1].V], WTokenArg[2].F / 100)))
    else if not HitOptions then writeln(ErrMess[11]);
    'HARRELLDAVISQUANTILE', 'HDQ':  if FoundVar(WTokenArg[1]) and (WTokenArg[2].F > 0.0) and (WTokenArg[2].F < 1.0) and (WTokenArg[3].S = EmptyStr) then
      writeln(WData[WTokenArg[1].V].Name, ' Harrell-Davis Quantile with p = ', SmartFloat(WTokenArg[2].F), ': ',
        SmartFloat(HarrellDavisQuantile(WData[WTokenArg[1].V], WTokenArg[2].F)))
    else writeln(ErrMess[17]);
    'PEARSONKMOMENT', 'PKM':  if FoundVar(WTokenArg[1]) and (WTokenArg[2].I > 0) and (WTokenArg[3].S = EmptyStr) then
      writeln(WData[WTokenArg[1].V].Name, ' Pearson''s Kth Moment with k = ', Trunc(WTokenArg[2].F), ': ',
        SmartFloat(PearsonKthMoment(WData[WTokenArg[1].V], Trunc(WTokenArg[2].F))))
    else writeln(ErrMess[12]);
    'QUICKSELECT', 'QS': if FoundVar(WTokenArg[1]) and (WTokenArg[2].I > 0) and (WTokenArg[3].S = EmptyStr) then
        writeln(WData[WTokenArg[1].V].Name, ' Kth Largest Element = ', SmartFloat(WTokenArg[1].F))
    else writeln(WTokenArg[i].S, ErrMess[12]);

    // Used in CLI to declare bariables and scalars.
    'DCL', 'DECLARE','DCLV', 'VAR', 'VARIABLE', 'CONST', 'CONSTANT', 'DCLC', 'CON': ;
...
procedure InitTolAndIter;
begin
  HighTolerance := CompilerHighTolerance;
  Tolerance := CompilerTolerance;
  LowTolerance := CompilerLowTolerance;
  UserTolerance := -99.0;
  UserMaxIter := -99;
end;
...
begin
  ...
  OSampleCutoff := SampleCutoff;
  OPermutations := Permutations;
end;
...
begin
  ...
  Reps := OReps;
  SampleSize := OSampleSize;
  SampleCutoff := OSampleCutoff;
  Permutations := OPermutations;
end;
...
procedure PutNullTokenArg;
begin
  // Make zeroth WTokenArg, Command, into Null.
  WTokenArg[iTA].F := NaN;
  WTokenArg[iTA].I := -99;
  WTokenArg[iTA].S := 'NULL';
  WTokenArg[iTA].V := -99;
end;
...
procedure PutTokenArg;
  ...
  if Code = 0 then begin
    m := trunc(n);
    WTokenArg[iTA].I := m;                        // M is integer; put it into Integer.
    //WTokenArg[iTA].V := -99;                     // -99 is sentinel to show unassigned.
  ...
  for i := 0 to nCol - 1 do
    if (m > 0) and (m <= nCol) then begin        // Is it in range?  N is one based, so subtract 1.
      WTokenArg[iTA].V := m - 1;                 // Put it in V.
      WTokenArg[iTA].S := WData[m - 1].Name;     // Put its name in S.
...
begin
  ...
  while true do begin               // Loop to read each Line.  Outer loop.
    ...
    InitTolAndIter;                 // Set user values to -99.
    BinCount := Ceil(Sqrt(nRow));   // Can't do earlier because need nRow.
    for i := 0 to MaxTokens do      // Initialize WTokens.
    ...
    if (Line = 'Q') or (Line = 'QUIT') or (Line = 'X') or (Line = 'EXIT') then Exit;
    ...
    // Loop to read each Token and put in WTokenArg. Inner Loop.
    repeat
      ...
      if DeclareVariable or DeclareScalar then begin
        ...
        Inc(Pos);
        if DebugOn then
          writeln('Sending to ParserExpression: *', Line, '* ', 'Pos = ', Pos);
        EvalLine := ParserExpression(Pos, ReturnPos, Line);           // Get result vector from Parser.  did return pos
        if DebugOn then begin
          writeln('Returning from ParserExpression: *', Line, '*', 'Pos = ', ReturnPos);
          //writeln('EvalLine length: ', Length(EvalLine.Value));
          DisplayVectorM(EvalLine.Name, EvalLine, T3);
        end;                                               // End detour to Parser, but need to add variable or scalar.
    ...
    OptionsCase;
    MainCase;
    if SlashMark > 0 then    // To deal with case where option like prec used as command.
      RestoreOptions;        // Always restore the original options. If not changes, no harm done.
