...
{ Save & Read Data }
procedure SavePartialData(const Mess: String; PProc: ProcType);
...
//procedure ModifyData(var WData: WMatrixType);
...
{procedure GeneralOptions1;
begin
      writeln('  P3  Display Data Large');
      writeln('  P2  Display Data Medium');
      writeln('  P1  Display Data Small');
      writeln('  Col Display Column');
      writeln('  O   Obtain or Modify Data');
      writeln('  F   Information on Program & Environment');
      writeln('  Opt Select Options');
      writeln('  S   Save Data');
      writeln('  H   Help');
      writeln('  D   Toggle Debug On/Off');
end;

procedure GeneralOptions2(const WInput: String);
begin
  Case WInput of
      'P3':  DisplayMatrixM('Main Data Large', WData, T3);
      'P2':  DisplayMatrixM('Main Data Medium', WData, T2);
      'P1':  DisplayMatrixM('Main Data Small', WData, T1);
      'Col': DisplayVectorM('', WData[0], T3);              //Input number of column
      'O':   ObtainData;
      'F':   begin ReportProgramState; Pause; end;
      'OPT': UserOptions;
      'S':   SaveData('main');
...
// ---------------------------------------------------------------
// Call this for messages that should go to console AND file
// ---------------------------------------------------------------
procedure Log(const Msg: string); overload;
var
  Timestamp: string;
begin
  WriteLn(Msg);  // normal console output
...
// ---------------------------------------------------------------
// Single procedure: True to start, False to stop
// ---------------------------------------------------------------
procedure LogToFile(StartLogging: Boolean; const FileName: string = '');
begin
  ...
  begin
    ...
    begin
      WriteLn('Warning: Logging already active.');
      Exit;
    end;
    ...
    if FileExists(FileName) then
      Append(LogFile)   // continue existing log
    else
      Rewrite(LogFile);

    if IOResult <> 0 then
    begin
      WriteLn('ERROR: Cannot open log file: ', FileName);
      Exit;
    end;

    LoggingActive := True;
    Log('>>> Logging started: ' + FileName);
  end
  else
  begin
    if not LoggingActive then
    begin
      WriteLn('Logging not active.');
      Exit;
    end;
...
var
  //WInput: String;
  i, j, m, iCol, nVar, iRow, iScal: Integer;
  TrimR, TrimL, Lambda, MultFactor, AddFactor: FloatType;
//  WVar: CIVectorType;
  NewVar: WVectorType;
  NewName: String;
  AddError: Boolean;
begin
  ...
  repeat
    ...
    writeln('  2  Center Data (Mean=0)');
    writeln('  3  Standardize Data (Mean=0 and SD=1)');
    writeln('  4  Winsorize Data (Truncate Observations)');
    writeln('  5  Yeo-Johnson Transform Data (Modify Observations)');
    writeln('  6  Create Linear Transform as New Variable');
    writeln('  7  Delete Variable (across all observation)');
    writeln('  8  Delete Row/Observation (across all variables)');
    writeln('  9  Delete Constant');
    GeneralOptions1;
    writeln('  X  Exit');
    write('>');
    readln(WInput);
    ...
    Case WInput of
      '1': begin
        ReadVariableNameOrNumber(iCol);
        write('Upper value: ');
        readln(TrimR);
        write('Lower value: ');
        readln(TrimL);
        DataTrim(iCol, TrimL, TrimR);
        writeln('Data have been trimmed.');
      end;
      '2': begin
        SelectColumns(WData, WVar, nVar);
        for i := 0 to nVar - 1 do
          CenterColumn(WData[WVar[i]]);
        writeln('Data have been centered.');
      end;
      '3': begin
        SelectColumns(WData, WVar, nVar);
        for i := 0 to nVar - 1 do
          StandardizeColumn(WData[WVar[i]]);
        writeln('Data have been standardized.');
      end;
     '4': begin
        ReadVariableNameOrNumber(iCol);
        repeat
          ...
          readln(TrimL);
        until (TrimL < 1.0) and (TrimL > 0.0);
        repeat
          write('Upper value between 0 and 1: ');
          readln(TrimR);
        until (TrimR > TrimL) and (TrimR < 1.0);
        WinsorizeWColumn(WData[iCol], TrimL, TrimR);
      ...
      '5': begin
        ...
        SelectColumns(WData, WVar, nVar);
        for i := 0 to nVar - 1 do
          YeoJohnsonTransform(WData[i], Lambda);
        writeln('Data have been transformed by Yeo-Johnson.');
      end;
      '6': begin
        ReadVariableNameOrNumber(iCol);
        m := Length(WData[iCol].Value);
        SetLength(NewVar.Value, m);
        //SetLength(WData, nCol + 1);
        //SetLength(WData[nCol].Value, m);
        ...
        readln(AddFactor);
        write('Name of new variable: ');
        readln(NewName);
        for j := 0 to m - 1 do begin
          NewVar.IsNan[j] := False;
          NewVar.Value[j] := WData[iCol].Value[j] * MultFactor + AddFactor;
        end;
        AddError := False;
        AddWVector(NewName, NewVar, AddError);
        if not AddError then begin
          writeln('New variable ', NewName,' has been added.');
          FillVectorCaches(WData[nCol]);
          Inc(nCol);
        end;
      end;
      '7': begin //Delete variable
        ReadVariableNameOrNumber(iCol);
        DeleteVariable(iCol);
      end;
      '8': begin //Delete row
      ...
      '9': begin //Delete constant
        ReadScalarNameOrNumber(iScal);
        DeleteScalar(iScal);
      end;
    end;
...
begin
  ...
  writeln('Data Entry Page');
  writeln;
  writeln('Options: ');
  writeln('  C Create Random Data');
  writeln('  T1 Test Data, 12 rows x 5 columns');
  writeln('  T2 Test Data, 2 rows x 2 columns');
  writeln('  T3 Test Data, 3 rows x 3 columns');
  writeln('  T4 Test Data, 5 rows x 3 columns');
  writeln('  T5 Test Data, 2 rows x 2 columns of zeroes');
  writeln('  T6 Test Data, 2 rows x 2 columns of ones');
  writeln('  T7 Test Data, 2 rows x 1 column of [2, 4]');
  writeln('  T8 Test Data, 1 row x 1 column of zero');
  writeln('  T9 Test Data, , 12 rows x 5 columns, big numbers');
  writeln('  R Read Data from CSV file');
  writeln('  M Manually Enter Data');
  ...
  case WInput of
    'C': begin
        //Repeat    // max rows is 999 and max cols is 99
         ...
         readln(nCol);
         //if (nCol <= 0) or (nCol > MaxCol) then writeln('Number is too big or too small.');
        //until (nCol > 0) and (nCol < MaxCol);
        //repeat
          write('Enter number of rows (observations): ');
          readln(nRow);
          //if (nCol <= 0) or (nCol > MaxRow) then writeln('Number is too big or too small.');
        //until (nRow > 0) and (nRow < MaxRow);
        //writeln;
        CreateRandomData;
        if Facade = MDI then Pause;
    end;
    'T1': begin
      GetTestData(WInput);
    end;
...
{      write('Enter number of variables: ');
      ...
      readln(nRow);}
      ReadCSVData(FileName, nRow, nCol);
    ...
    'O': begin
      if not ((nRow = 0) and (nCol = 0)) then begin
        ModifyData(WData);
        nRow := Length(WData[0].Value);
        nCol := Length(WData);
      end;
    end;
    'J': begin
