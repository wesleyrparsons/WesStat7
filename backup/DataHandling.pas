...
procedure DisplayCorrelationMatrix(const CData, SData: TMatrix; const WVar: TIVector;
  const IsPearson, PrintSig: Boolean; const CorrMess: TCorr);
procedure DisplayCovarianceMatrix(const CVData: TMatrix; const WVar: TIVector);
...
procedure DisplayDataA(const PData: TMatrix; WData: TString);
...
begin
  for j := 0 to High(Data) do
    for i := 0 to High(Data[0]) do begin
      Data[j, 2] := abs(Data[j, 2]) + 5.0;  //  col 3 > 0
      Data[j, 1] := Data[j, 0] + Random;  //highly correlated
    end;
end;
...
procedure DisplayDataA(const PData: TMatrix; WData.Name: TString);
var
  i, j, m, n: Integer;
begin
  m := Length(PData);
  ...
  writeln('Data     Obs: ', m, '   Vars: ', n);
  Write('  Var');
  for i := 0 to n - 1 do write('    ', WData.Name[i] : 8);
  writeln;
  for j := 0 to m - 1 do begin
    Write('Obs', j + 1: 3);
    for i := 0 to n - 1 do
...
begin
  m := Length(WVar);
  for j := 0 to m -1 do Col[j] := WData[j, WVar[0]];
  writeln(WData.Name[WVar[0]]);
  for j := 0 to m - 1 do
    write('  ', Col[j]: 5: 2);
  writeln;
  Pause;
  ...
  if UpCase(QInput) = 'Y' then begin
    ...
    for j := 0 to High(Data) do begin
      for i := 0 to High(Data[j]) do begin
        write(f, Data[j][i]: 0: 6);
        if i < High(Data[i]) then write(f, ',');
      end;
      writeln(f);
    end;
    closefile(f);
...
var                     // winsorizes column iCol - 1
  ...
  j, m, lowerIdx, upperIdx: Integer;
  Col: TVector;
begin
  m := Length(Data);
  GetColumn(Data, iCol - 1, Col);
  //Sort
  QuickSort(Col, 0, High(Col));
  // Step 2: Determine bounds
  lowerIdx := Round(lowerPct * m);
  upperIdx := Round(upperPct * m) - 1;
  lowerIdx := Max(0, Min(m - 1, lowerIdx));
  upperIdx := Max(0, Min(m - 1, upperIdx));
  lowerBound := Col[lowerIdx];
  upperBound := Col[upperIdx];
  // Step 3: Winsorize original Data
  for j := 0 to m - 1 do begin
    if Col[j] < lowerBound then
      Col[j] := lowerBound
    else if Col[j] > upperBound then
      Col[j] := upperBound;
  end;
...
begin
  ...
  for j := 0 to m - 1 do begin
    ...
    if xi >= 0 then begin
      if Lambda = 0 then
        Data[j, iCol - 1] := Ln(xi + 1)
      else
        Data[j, iCol - 1] := (Power(xi + 1, Lambda) - 1) / Lambda;
    end
    else begin
      if Lambda = 2 then
        Data[j, iCol - 1] := -Ln(-xi + 1)
      else
        Data[j, iCol - 1] := - (Power(-xi + 1, 2 - Lambda) - 1) / (2 - Lambda);
    end;
  end;
end;
...
begin
  Case Str of
    ...
    'T3': begin
      nRow := 3;
      nCol := 3;
      SetLength(WData, nRow, nCol);             //invertible det =-9
      SetLength(WData.Name, nCol);
      ...
      WData[2,0] := 5; WData[2,1] := 2; WData[2,2] := 1;
      writeln('Data created successfully.');
    end;
    'T4': begin
      nRow := 5;
