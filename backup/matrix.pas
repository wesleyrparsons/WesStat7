...
USES Math, MathFunc, Vector, Alglib;
...
TYPE
  MatrixStruc = RECORD
    Columns, Rows: WORD;
    Data.Value: ARRAY[1..MaxVector] OF float;
  END;
...
PROCEDURE CenterData.Value(VAR A: MatrixTyp);
...
BEGIN
  ...
  IF (Row <= n) AND (Column <= p)
    THEN
      GetMatrixElement := A^.Data.Value[Pred(Row) * p + Column]
    ELSE
      BEGIN
        MatrixError := true;
...
BEGIN
  ...
  IF (Row <= n) AND (Column <= p)
    THEN
      A^.Data.Value[Pred(Row) * p + Column] := Value
    ELSE
      BEGIN
        MatrixError := true;
...
VAR
  ...
  VAR DetEqualsMaxError: BOOLEAN);
     {- This procedure searches the ReferenceRow column of the matrix Data.Value for
...
BEGIN
  ...
  FOR i := 1 TO n DO
    BEGIN
      IF IsNaN(GetMatrixElement(t, i, Column))    // check forR NaN Data.Value
        THEN INC(NaNs)
        ELSE IF (GetMatrixElement(t, i, Column)) > s
               THEN
                 s := GetMatrixElement(t, i, Column); // and find largest element of Data.Value vector
    END;
  s := 10 * s;
  IF (NaNs > 0)
    THEN
